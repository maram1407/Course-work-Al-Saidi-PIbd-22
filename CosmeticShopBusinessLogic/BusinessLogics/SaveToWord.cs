...
namespace CosmeticShopBusinessLogic.BusinessLogic
{
                ...
                if (info.Orders != null)
                {
                    foreach (var orders in info.Orders)
                    {
                        if ((orders.Status == Enums.Status.Готов) || (orders.Status == Enums.Status.Оплачен))
                        {
                            ...
                            docBody.AppendChild(CreateParagraph(new WordParagraph
                            {
                                Texts = new List<string> {  "Набор: " + orders.SetName},
                                TextProperties = new WordParagraphProperties
                                {
                                    Bold = false,
                                    Size = "24",
                            ...
                            docBody.AppendChild(CreateParagraph(new WordParagraph
                            {
                                Texts = new List<string> { "Косметика: " },
                                TextProperties = new WordParagraphProperties
                                {
                                    Bold = false,
                                    Size = "24",
                            ...
                            foreach (var set in info.SetCosmetics)
                            {
                                if (set.SetName == orders.SetName)
                                {
                                    docBody.AppendChild(CreateParagraph(new WordParagraph
                                    {
                                        Texts = new List<string> { set.CosmeticName },
                                        TextProperties = new WordParagraphProperties
                                        {
                                            Bold = false,
                                            Size = "24",

...
namespace CosmeticShopView
{
    public partial class FormCreateOrder : Form
    {
        ...
        private readonly ISetLogic logicP;
        ...
        public FormCreateOrder(ISetLogic logicP, MainLogic logicM,
        ...
        private void FormCreateOrder_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<SetViewModel> listDishses = logicP.Read(null);
                if (listDishses != null)
                {
                    comboBoxSet.DisplayMember = "SetName";
                    comboBoxSet.ValueMember = "Id";
                    comboBoxSet.DataSource = listDishses;
        ...
        private void CalcSum()
        {
            if (comboBoxSet.SelectedValue != null && !string.IsNullOrEmpty(textBoxCount.Text))
            {
                try
                {
                    ...
                    SetViewModel product = logicP.Read(new SetBindingModel
                    {
                        Id = id
                    })?[0];
                    int count = Convert.ToInt32(textBoxCount.Text);
                    textBoxSum.Text = (count * product?.Price ?? 0).ToString();
                    foreach (var p in product.SetCosmetics)
                    {
                        requestCosmetics.Add(p.Key, (product.SetCosmetics[p.Key].Item1, product.SetCosmetics[p.Key].Item2, false));
                    }
                }
                catch (Exception ex)
                {

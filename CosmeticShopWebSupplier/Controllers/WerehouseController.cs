...
namespace CosmeticShopWebSupplier.Controllers
{
    public class WerehouseController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult ChangeWerehouse(int id, [Bind("Id,FridgeName,Capacity,Type")] Werehouse fridge)
        {
            if (Program.Supplier == null)
            {
                return new UnauthorizedResult();
        ...
        public IActionResult ReserveCosmeticId(int werehouseId, int cosmeticId, int count, int requestId)
        {
            if (Program.Supplier == null)
            {
                return new UnauthorizedResult();
